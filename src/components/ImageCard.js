...
const ImageCard = ({image}) => {
  const tags = image.tags.split(',');
    ...
    <div className="max-w-sm rounded oveflow-hidden shadow-lg">
      <img src={image.webformatURL} alt="" className="w-1/3"/>
      ...
      <div className="px-6 py-4">
        {tags.map(tag => (
            <span key={image.id} className="inline-block bg-blue-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2">
              #{tag}
            </span>
        ))}
      </div>
